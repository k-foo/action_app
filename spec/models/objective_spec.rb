  ...
  describe 'WOOPの法則を浸かった目標の設定' do
    ...
    context '目標設定ができないとき' do
      it 'タイトルが空欄だと登録できない' do
        @objective.title = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include("Title can't be blank")
      end
      it 'カテゴリーの情報が空欄だと登録できない' do
        @objective.category_id = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include("Category can't be blank")
      end
      it '「願望」部分が空欄だと登録できない' do
        @objective.wish = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include("Wish can't be blank")
      end
      it '「結果」部分が空欄だと登録できない' do
        @objective.outcome = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include("Outcome can't be blank")
      end
      it '「障害」部分が空欄だと登録できない' do
        @objective.obstacle = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include("Obstacle can't be blank")
      end
      it '「計画」部分が空欄だと登録できない' do
        @objective.plan = nil
        @objective.valid?
        expect(@objective.errors.full_messages).to include("Plan can't be blank")
      end
    end
  end
end
